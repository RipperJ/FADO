...
def get_ap_rst_pipeline_def(slot_list: List[Slot], ap_rst:bool, pipeline_level=4) -> List[str]:
  pipeline_def = []
  ...
  for slot in slot_list:
    ...
    pipeline_def.append(f'always @ (posedge ap_clk) begin')
    pipeline_def.append(f'  ap_rst_{slot.getRTLModuleName()}_q0 <= {"ap_rst" if ap_rst else "!ap_rst_n"};')
    for level in range(1, pipeline_level):
...
def remove_orig_ctrl_signal(temp_rtl_top: List[str]) -> None:
  ...
  for i in range(len(temp_rtl_top)):
    ...
    if 'assign' in temp_rtl_top[i] and any(ctrl_signal in temp_rtl_top[i] for ctrl_signal in ctrl_signals):
      temp_rtl_top[i] = f'//  {temp_rtl_top[i]}'
