...
class SlotManager:
  ...
  def getAllFourByFourCRSlots(self):
    # ! DLF: Get all slots of 4x4 clock regions for U250
    slot_list = []
    for x in range(0, 8, 4):
      for y in range(0, 16, 4):
        slot_list.append(Slot(self.board, f'CLOCKREGION_X{x}Y{y}:CLOCKREGION_X{x+3}Y{y+3}'))
    return slot_list
  ...
  def getBottomAndUpSplit(self, slot:Slot):
