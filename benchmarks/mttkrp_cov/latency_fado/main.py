...
from DSE_mttkrp_cov import DSE_mttkrp_cov
...
PREFIX_REGEXS_TO_FUNC_LIBNAME = {
    ...
    r"C_IO_L3_in_x": "C_IO_L3_in",
    r"C_IO_L3_in_serialize_x": "C_IO_L3_in_serialize",
    r"C_IO_L2_in_\d+_x": "C_IO_L2_in",
    r"C_IO_L2_in_boundary_x": "C_IO_L2_in_boundary",
    r"PE_wrapper_\d+_\d+": "PE_wrapper",
    r"A_PE_dummy_in_\d+_x": "A_PE_dummy_in",
    r"B_PE_dummy_in_\d+_x": "B_PE_dummy_in",
    r"C_PE_dummy_in_\d+_x": "C_PE_dummy_in",
    r"D_drain_IO_L1_out_wrapper_\d+_\d+": "D_drain_IO_L1_out_wrapper",
    r"D_drain_IO_L1_out_boundary_wrapper_\d+": "D_drain_IO_L1_out_boundary_wrapper",
    r"D_drain_IO_L2_out_\d+_x": "D_drain_IO_L2_out",
    r"D_drain_IO_L2_out_boundary_x": "D_drain_IO_L2_out_boundary",
    r"D_drain_IO_L3_out_x": "D_drain_IO_L3_out",
    r"D_drain_IO_L3_out_serialize_x": "D_drain_IO_L3_out_serialize",
...
env LC_ALL=C v++ \\
  ...
  --connectivity.sp ${TOP}_1.D:DDR[3] \\
  --kernel_frequency %s \\
...
def parseUserConstraints(abconfig, board:DeviceBase, graph:DataflowGraph, slot_manager:SlotManager):
    ...
    if "Floorplan" in abconfig:
        ...
        for v in graph.getAllVertices():
            if "axi_U" not in v.name and v.name != "D_drain_IO_L3_out_serialize_x1_U0" and v.name != "C_IO_L3_in_serialize_x1_U0":   # ! gmem_C_m_axi_U on the other die, so the grouped signal is excluded
...
if __name__ == "__main__":
    ...
    if "9" in sys.argv:
        ...
        dse_engine = DSE_mttkrp_cov(board, top_rtl_parser, slot_manager, floorplan, graph, DSE_LIB_JSON, PREFIX_REGEXS_TO_FUNC_LIBNAME, abconfig["AreaUtilizationRatio"], sub_rtl_paths, 5, abconfig["FloorplanMethod"])
    ...
    if "10" in sys.argv:
        ...
        for LAnum in range(0, 65):
            ...
            dse_engine = DSE_mttkrp_cov(board, top_rtl_parser, slot_manager, floorplan, graph, DSE_LIB_JSON, PREFIX_REGEXS_TO_FUNC_LIBNAME, abconfig["AreaUtilizationRatio"], sub_rtl_paths, LAnum)
            logging.info("DSE strategy: {}".format(DSE_STRATEGY))
