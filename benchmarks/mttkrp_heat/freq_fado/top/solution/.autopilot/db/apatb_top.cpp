...
// wrapc file define:
#define AUTOTB_TVIN_gmem_D "../tv/cdatafile/c.top.autotvin_gmem_D.dat"
#define AUTOTB_TVOUT_gmem_D "../tv/cdatafile/c.top.autotvout_gmem_D.dat"
// wrapc file define:
...
// tvout file define:
#define AUTOTB_TVOUT_PC_gmem_D "../tv/rtldatafile/rtl.top.autotvout_gmem_D.dat"
// tvout file define:
...
string get_depth_list () {
  ...
  total_list << "{gmem_D " << gmem_D_depth << "}\n";
...
extern "C" void top_hw_stub_wrapper(volatile void *, volatile void *, volatile void *, volatile void *);
  ...
  if (wrapc_switch_file_token.good())
  {
    ...
    string AESL_num;{
      ...
      if (!rtl_tv_out_file.is_open()) {
        rtl_tv_out_file.open(AUTOTB_TVOUT_PC_gmem_D);
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<512> > gmem_D_pc_buffer(1);
          int i = 0;

          rtl_tv_out_file >> AESL_token; //data
          while (AESL_token != "[[/transaction]]"){

            RTLOutputCheckAndReplacement(AESL_token, "gmem_D");
  
            // push token into output port buffer
            if (AESL_token != "") {
              gmem_D_pc_buffer[i] = AESL_token.c_str();;
              i++;
            }
          ...
          if (i > 0) {{
            int i = 0;
            for (int j = 0, e = 1; j < e; j += 1, ++i) {((long long*)__xlx_apatb_param_D)[j*8+0] = gmem_D_pc_buffer[i].range(63,0).to_int64();
((long long*)__xlx_apatb_param_D)[j*8+1] = gmem_D_pc_buffer[i].range(127,64).to_int64();
((long long*)__xlx_apatb_param_D)[j*8+2] = gmem_D_pc_buffer[i].range(191,128).to_int64();
((long long*)__xlx_apatb_param_D)[j*8+3] = gmem_D_pc_buffer[i].range(255,192).to_int64();
((long long*)__xlx_apatb_param_D)[j*8+4] = gmem_D_pc_buffer[i].range(319,256).to_int64();
((long long*)__xlx_apatb_param_D)[j*8+5] = gmem_D_pc_buffer[i].range(383,320).to_int64();
((long long*)__xlx_apatb_param_D)[j*8+6] = gmem_D_pc_buffer[i].range(447,384).to_int64();
((long long*)__xlx_apatb_param_D)[j*8+7] = gmem_D_pc_buffer[i].range(511,448).to_int64();
}}}
        } // end transaction
      } // end file is good
...
//gmem_D
aesl_fh.touch(AUTOTB_TVIN_gmem_D);
aesl_fh.touch(AUTOTB_TVOUT_gmem_D);
//A
...
// print gmem_D Transactions
{
  sprintf(__xlx_sprintf_buffer.data(), "[[transaction]] %d\n", AESL_transaction);
  aesl_fh.write(AUTOTB_TVIN_gmem_D, __xlx_sprintf_buffer.data());
  {  __xlx_offset_byte_param_D = 0*64;
  if (__xlx_apatb_param_D) {
    for (int j = 0  - 0, e = 1 - 0; j != e; ++j) {
sc_bv<512> __xlx_tmp_lv;
__xlx_tmp_lv.range(63,0) = ((long long*)__xlx_apatb_param_D)[j*8+0];
__xlx_tmp_lv.range(127,64) = ((long long*)__xlx_apatb_param_D)[j*8+1];
__xlx_tmp_lv.range(191,128) = ((long long*)__xlx_apatb_param_D)[j*8+2];
__xlx_tmp_lv.range(255,192) = ((long long*)__xlx_apatb_param_D)[j*8+3];
__xlx_tmp_lv.range(319,256) = ((long long*)__xlx_apatb_param_D)[j*8+4];
__xlx_tmp_lv.range(383,320) = ((long long*)__xlx_apatb_param_D)[j*8+5];
__xlx_tmp_lv.range(447,384) = ((long long*)__xlx_apatb_param_D)[j*8+6];
__xlx_tmp_lv.range(511,448) = ((long long*)__xlx_apatb_param_D)[j*8+7];
  ...
  tcl_file.set_num(1, &tcl_file.gmem_D_depth);
  sprintf(__xlx_sprintf_buffer.data(), "[[/transaction]] \n");
  aesl_fh.write(AUTOTB_TVIN_gmem_D, __xlx_sprintf_buffer.data());
}
...
// print D Transactions
{
  ...
  {
    sc_bv<64> __xlx_tmp_lv = __xlx_offset_byte_param_D;
...
top_hw_stub_wrapper(__xlx_apatb_param_A, __xlx_apatb_param_B, __xlx_apatb_param_C, __xlx_apatb_param_D);
CodeState = DUMP_OUTPUTS;
// print gmem_D Transactions
{
  sprintf(__xlx_sprintf_buffer.data(), "[[transaction]] %d\n", AESL_transaction);
  aesl_fh.write(AUTOTB_TVOUT_gmem_D, __xlx_sprintf_buffer.data());
  {  __xlx_offset_byte_param_D = 0*64;
  if (__xlx_apatb_param_D) {
    for (int j = 0  - 0, e = 1 - 0; j != e; ++j) {
sc_bv<512> __xlx_tmp_lv;
