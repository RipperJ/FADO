...
if __name__ == "__main__":
    ...
    if "9" in sys.argv:
        ...
        dse_engine = DSE_mttkrp(board, top_rtl_parser, slot_manager, floorplan, graph, DSE_LIB_JSON, PREFIX_REGEXS_TO_FUNC_LIBNAME, abconfig["AreaUtilizationRatio"], sub_rtl_paths, 7, abconfig["FloorplanMethod"])
        logging.info("DSE strategy: {}".format(DSE_STRATEGY))
        dse_engine.dse(DSE_STRATEGY)
        
        os.system("mv directives.tcl d_old.tcl")
        old_tcl = open("d_old.tcl", "r").readlines()
        with open("directives.tcl", "w") as new_tcl:
            for line in old_tcl:
                if re.search("nondf_kernel", line) or re.search('bind_storage "A_IO_L2_in', line) or re.search('bind_storage "C_IO_L2_in_[0-2]_x1', line):
                    new_tcl.write(line.replace("bram", "uram"))
                else:
                    new_tcl.write(line)
        exit()
        # run_hls(
    ...
    if "10" in sys.argv:
        ...
        for LAnum in range(0, 65):
            ...
            dse_engine = DSE_mttkrp(board, top_rtl_parser, slot_manager, floorplan, graph, DSE_LIB_JSON, PREFIX_REGEXS_TO_FUNC_LIBNAME, abconfig["AreaUtilizationRatio"], sub_rtl_paths, LAnum)
            logging.info("DSE strategy: {}".format(DSE_STRATEGY))
