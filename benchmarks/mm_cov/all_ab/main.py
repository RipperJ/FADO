...
PREFIX_REGEXS_TO_FUNC_LIBNAME = {
    ...
    r"nondf_kernel_cov": "nondf_kernel_cov"
}
...
def parseUserConstraints(abconfig, board:DeviceBase, graph:DataflowGraph, slot_manager:SlotManager):
    ...
    if "Floorplan" in abconfig:
        ...
        for v in graph.getAllVertices():
            if "axi_U" not in v.name and v.name != "C_drain_IO_L3_out_serialize_x0_U0":   # ! gmem_C_m_axi_U on the other die, so the grouped signal is excluded
...
if __name__ == "__main__":
    ...
    if "9" in sys.argv:
        ...
        dse_engine = DSE_mm(board, top_rtl_parser, slot_manager, floorplan, graph, DSE_LIB_JSON, PREFIX_REGEXS_TO_FUNC_LIBNAME, abconfig["AreaUtilizationRatio"], sub_rtl_paths, 46, abconfig["FloorplanMethod"])
    ...
    if "10" in sys.argv:
        ...
        for LAnum in range(0, 65):
            ...
            dse_engine = DSE_mm(board, top_rtl_parser, slot_manager, floorplan, graph, DSE_LIB_JSON, PREFIX_REGEXS_TO_FUNC_LIBNAME, abconfig["AreaUtilizationRatio"], sub_rtl_paths, LAnum)
            logging.info("DSE strategy: {}".format(DSE_STRATEGY))
