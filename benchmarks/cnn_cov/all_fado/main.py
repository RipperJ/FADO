...
from DSE_cnn_cov import DSE_cnn_cov
...
DSE_STRATEGY = "incrfp" # choice: incrfp, nofp, globalfp, all, LAtest
...
PREFIX_REGEXS_TO_FUNC_LIBNAME = {
    ...
    r"B_IO_L2_in_x": "B_IO_L2_in",
    r"B_IO_L2_in_boundary_x": "B_IO_L2_in_boundary",
    r"PE_wrapper_\d+_\d+": "PE_wrapper",
    r"A_PE_dummy_\d+_x": "A_PE_dummy",
    r"B_PE_dummy_\d+_x": "B_PE_dummy",
    r"C_drain_IO_L1_out_boundary_\d+_x": "C_drain_IO_L1_out_boundary",
    r"C_drain_IO_L1_out_\d+_x": "C_drain_IO_L1_out",
    r"C_drain_IO_L2_out_boundary_x": "C_drain_IO_L2_out_boundary",
    r"C_drain_IO_L2_out_x": "C_drain_IO_L2_out",
    r"C_drain_IO_L3_out_x": "C_drain_IO_L3_out",
    r"nondf_kernel_cov_x": "nondf_kernel_cov"
}

ABCONFIG = '''
{
    "FloorplanMethod": "IterativeDivisionToHalfSLR",
    "AreaUtilizationRatio" : 0.64,
...
def parseUserConstraints(abconfig, board:DeviceBase, graph:DataflowGraph, slot_manager:SlotManager):
    ...
    if "Floorplan" in abconfig:
        ...
        for v in graph.getAllVertices():
            logging.info("v.name: {}".format(v.name))
            if "axi_U" not in v.name and v.name != "C_drain_IO_L3_out_x2_U0":   # ! gmem_C_m_axi_U on the other die, so the grouped signal is excluded
...
def draw_dataflow_graph(graph:DataflowGraph):
    ...
    for v_node in graph.top_rtl_parser.traverseVertexInAST():
        if v_node.name in graph.hls_prj_manager.getSubInst() or v_node.name.find("data_split") != -1: # exclude those sub-functions' name
...
if __name__ == "__main__":
    ...
    if "6" in sys.argv:
        ...
        with open("cnn_template.cpp", "r") as src_template:
            ...
            for i in range(len(lines)):
                if regex_func_def.match(lines[i]):
                    # to search for fifos in "kernel3"
                    if lines[i].find("kernel3") != -1:
                        in_kernel = 1
                    else:
                        in_kernel = 0
    ...
    if "9" in sys.argv:
        ...
        dse_engine = DSE_cnn_cov(board, top_rtl_parser, slot_manager, floorplan, graph, DSE_LIB_JSON, PREFIX_REGEXS_TO_FUNC_LIBNAME, abconfig["AreaUtilizationRatio"], sub_rtl_paths, 3, abconfig["FloorplanMethod"])
    ...
    if "10" in sys.argv:
        ...
        for LAnum in range(0, 65):
            ...
            dse_engine = DSE_cnn_cov(board, top_rtl_parser, slot_manager, floorplan, graph, DSE_LIB_JSON, PREFIX_REGEXS_TO_FUNC_LIBNAME, abconfig["AreaUtilizationRatio"], sub_rtl_paths, LAnum)
            logging.info("DSE strategy: {}".format(DSE_STRATEGY))
