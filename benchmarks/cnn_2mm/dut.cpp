...
void nondf_kernel_2mm(DATA_TYPE alpha, DATA_TYPE beta, DATA_TYPE xout[1024], DATA_TYPE xin[1024]) {
...
#pragma HLS INTERFACE ap_memory port = xin
    ...
    DATA_TYPE B[32][32];
    DATA_TYPE C[32][32];
    DATA_TYPE D_input[32][32];
    DATA_TYPE D_output[32][32];
    /* Input Util */
nondf_kernel_2mm_loop_1:     for (int i = 0; i < 32; i++) {     // SKIP_DSE
nondf_kernel_2mm_loop_2:         for (int j = 0; j < 32; j++) { // SKIP_DSE
            tmp[i][j] = xout[32 * i + j];
            A[i][j] = xout[32 * i + j];
            B[i][j] = xout[32 * i + j];
            C[i][j] = xout[32 * i + j];
            D_input[i][j] = xout[32 * i + j];
        }
    }

    /* D := alpha*A*B*C + beta*D */
nondf_kernel_2mm_loop_3:     for (i = 0; i < 32; i++)     // {"tmp": 1, "A": 1}
nondf_kernel_2mm_loop_4:         for (j = 0; j < 32; j++) // {"tmp": 2, "B": 2}
        {
            tmp[i][j] = 0;
nondf_kernel_2mm_loop_5:             for (k = 0; k < 32; ++k) // {"A": 2, "B": 1}
                tmp[i][j] += alpha * A[i][k] * B[k][j];
        }
nondf_kernel_2mm_loop_6:     for (i = 0; i < 32; i++)     // {"D_input": 1, "D_output": 1, "tmp": 1}
nondf_kernel_2mm_loop_7:         for (j = 0; j < 32; j++) // {"D_input": 2, "D_output": 2, "C": 2}
        {
            int sum = D_input[i][j] * beta;
nondf_kernel_2mm_loop_8:             for (k = 0; k < 32; ++k) // {"tmp": 2, "C": 1}
                sum += tmp[i][k] * C[k][j];
            D_output[i][j] = sum;
        }

    /* Output Util */
nondf_kernel_2mm_loop_9:     for (int i = 0; i < 32; i++) {     // SKIP_DSE
nondf_kernel_2mm_loop_10:         for (int j = 0; j < 32; j++) { // SKIP_DSE
            xin[32 * i + j] = D_output[i][j];
        }
    }
}
...
A_IO_L3_in_x0_loop_1:     for (ap_uint<13> i = 0; i < 1024; i++) {
        A_t16 mem_data;
        A_t8 fifo_data;
        mem_data = A[i];
...
B_IO_L3_in_x0_loop_1:     for (ap_uint<12> i = 0; i < 1024; i++) {
        B_t16 mem_data;
        B_t8 fifo_data;
        mem_data = B[i];
...
C_drain_IO_L3_out_x0_loop_1:     for (ap_uint<12> i = 0; i < 1024; i++) {
        C_t16 mem_data;
        C_t2 fifo_data;
...
A_IO_L3_in_x1_loop_1:     for (ap_uint<13> i = 0; i < 1024; i++) {
        A_t16 mem_data;
        A_t8 fifo_data;
        mem_data = A[i];
...
B_IO_L3_in_x1_loop_1:     for (ap_uint<12> i = 0; i < 1024; i++) {
        B_t16 mem_data;
        B_t8 fifo_data;
        mem_data = B[i];
...
C_drain_IO_L3_out_x1_loop_1:     for (ap_uint<12> i = 0; i < 1024; i++) {
        C_t16 mem_data;
        C_t2 fifo_data;
        C_t2 mem_data_split[8];
...
#pragma HLS INTERFACE m_axi port = C offset = slave bundle = gmem_C depth = 1024
#pragma HLS INTERFACE s_axilite port = A bundle = control
#pragma HLS INTERFACE s_axilite port = B bundle = control
#pragma HLS INTERFACE s_axilite port = C bundle = control
#pragma HLS INTERFACE s_axilite port = return bundle = control

    DATA_TYPE temp_xout0[1024], temp_xin1[1024];
    DATA_TYPE temp_tmp1[32][32], temp_A1[32][32], temp_B1[32][32], temp_C1[32][32], temp_Din1[32][32], temp_Dout1[32][32];
    kernel3_x0(A, B, temp_xout0);
    nondf_kernel_2mm(1, 1, temp_xout0, temp_xin1);
    kernel3_x1(temp_xin1, temp_xin1, C);
}
}
