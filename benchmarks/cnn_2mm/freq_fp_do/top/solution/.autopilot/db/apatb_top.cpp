    ...
    string AESL_num;{
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<512> > gmem_C_pc_buffer(1024);
          int i = 0;
          ...
          if (i > 0) {{
            int i = 0;
            for (int j = 0, e = 1024; j < e; j += 1, ++i) {((long long*)__xlx_apatb_param_C)[j*8+0] = gmem_C_pc_buffer[i].range(63,0).to_int64();
...
__xlx_tmp_lv.range(511,448) = ((long long*)__xlx_apatb_param_A)[j*8+7];
  ...
  tcl_file.set_num(1024, &tcl_file.gmem_A_depth);
...
__xlx_tmp_lv.range(511,448) = ((long long*)__xlx_apatb_param_B)[j*8+7];
  ...
  tcl_file.set_num(1024, &tcl_file.gmem_B_depth);
  sprintf(__xlx_sprintf_buffer.data(), "[[/transaction]] \n");
  aesl_fh.write(AUTOTB_TVIN_gmem_B, __xlx_sprintf_buffer.data());
}
...
__xlx_tmp_lv.range(511,448) = ((long long*)__xlx_apatb_param_C)[j*8+7];
  ...
  tcl_file.set_num(1024, &tcl_file.gmem_C_depth);
  sprintf(__xlx_sprintf_buffer.data(), "[[/transaction]] \n");
  aesl_fh.write(AUTOTB_TVIN_gmem_C, __xlx_sprintf_buffer.data());
}
