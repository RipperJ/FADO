...
DSE_STRATEGY = "globalfp" # incrfp, nofp, globalfp, all, LAtest
...
if __name__ == "__main__":
    ...
    if "2" in sys.argv:
        print("> 0. Making Backup for HLS Logs")
        if os.path.exists("./{}/".format(TOP_FUNC)):
            os.system("rm -rf ./{}/".format(TOP_FUNC))
        os.system("cp -r ./{}_base ./{}".format(TOP_FUNC, TOP_FUNC))
    ...
    if "9" in sys.argv:
        ...
        dse_engine = DSE_mm2mm(board, top_rtl_parser, slot_manager, floorplan, graph, DSE_LIB_JSON, PREFIX_REGEXS_TO_FUNC_LIBNAME, abconfig["AreaUtilizationRatio"], sub_rtl_paths, 28, abconfig["FloorplanMethod"])
        logging.info("DSE strategy: {}".format(DSE_STRATEGY))
        dse_engine.dse(DSE_STRATEGY)
        logging.info("Re-synthesize with optimized directives.")
        os.system("vitis_hls -f generate_rtl.tcl")
        for sub_rtl_path in dse_engine.sub_rtl_paths:
            logging.info("Replacing {}/{}/syn/verilog/{}.".format(TOP_FUNC, SOLUTION, dse_engine.top_rtl_parser.getModuleName(sub_rtl_path)))
            os.system("rm {}/{}/syn/verilog/{}".format(TOP_FUNC, SOLUTION, dse_engine.top_rtl_parser.getModuleName(sub_rtl_path)))
            # print(dse_engine.top_rtl_parser.getModuleName(sub_rtl_path))
            os.system("cp {} {}/{}/syn/verilog/{}".format(dse_engine.top_rtl_parser.getModuleName(sub_rtl_path), TOP_FUNC, SOLUTION, dse_engine.top_rtl_parser.getModuleName(sub_rtl_path)))
        logging.info("Updating 'entry.*' in constraint file.")
        constraint = open("constraint.tcl", "r").readlines()
        with open("temp_constraint.tcl", "w") as temp_constraint:
            for line in constraint:
                temp_constraint.write(re.sub("entry\d+", 'entry.*', line))
        os.system("rm constraint.tcl")
        os.system("mv temp_constraint.tcl constraint.tcl")
        
        # old_entry = open("{}/{}/syn/verilog/top_kernel0_x0_entry83.v".format(TOP_FUNC, SOLUTION), "r").readlines()
        # with open("{}/{}/syn/verilog/top_kernel0_x0_entry9.v".format(TOP_FUNC, SOLUTION), "w") as new_entry:
        #     for line in old_entry:
        #         new_entry.write(re.sub("entry83", "entry9", line))
        # old_entry = open("{}/{}/syn/verilog/top_kernel0_x1_entry86.v".format(TOP_FUNC, SOLUTION), "r").readlines()
        # with open("{}/{}/syn/verilog/top_kernel0_x1_entry12.v".format(TOP_FUNC, SOLUTION), "w") as new_entry:
        #     for line in old_entry:
        #         new_entry.write(re.sub("entry86", "entry12", line))
                
        os.system("mv {}/{}/syn/verilog/top_kernel0_x0.v {}/{}/syn/verilog/top_kernel0_x0_backup.v".format(TOP_FUNC, SOLUTION, TOP_FUNC, SOLUTION))
        old_entry = open("{}/{}/syn/verilog/top_kernel0_x0_backup.v".format(TOP_FUNC, SOLUTION), "r").readlines()
        with open("{}/{}/syn/verilog/top_kernel0_x0.v".format(TOP_FUNC, SOLUTION), "w") as new_entry:
            for line in old_entry:
                new_entry.write(re.sub("entry9", "entry83", line))
                
        os.system("mv {}/{}/syn/verilog/top_kernel0_x1.v {}/{}/syn/verilog/top_kernel0_x1_backup.v".format(TOP_FUNC, SOLUTION, TOP_FUNC, SOLUTION))
        old_entry = open("{}/{}/syn/verilog/top_kernel0_x1_backup.v".format(TOP_FUNC, SOLUTION), "r").readlines()
        with open("{}/{}/syn/verilog/top_kernel0_x1.v".format(TOP_FUNC, SOLUTION), "w") as new_entry:
            for line in old_entry:
                new_entry.write(re.sub("entry12", "entry86", line))
        exit()
        # run_hls(
        #     DUT_FILE,
        #     TOP_FUNC,
    ...
    if "10" in sys.argv:
        ...
        for LAnum in range(0, 65):
            ...
            dse_engine = DSE_mm2mm(board, top_rtl_parser, slot_manager, floorplan, graph, DSE_LIB_JSON, PREFIX_REGEXS_TO_FUNC_LIBNAME, abconfig["AreaUtilizationRatio"], sub_rtl_paths, LAnum)
            logging.info("DSE strategy: {}".format(DSE_STRATEGY))
            QoR = dse_engine.dse(DSE_STRATEGY)
            
            R.append(QoR[0])
            L.append(QoR[1])
            SearchTime.append(QoR[2])
        
        logging.info("R: {}".format(R))
        logging.info("L: {}".format(L))
        logging.info("SearchTime: {}".format(SearchTime))